...
(function($) {
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		var navRow = jQuery('<tr><td colspan="7"><div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix FormHeader MonthNavigation"></div></td></tr>').css({ "height" : defaults.navHeight });
		...
		for (var i=defaults.firstDayOfWeek; i<defaults.firstDayOfWeek+7; i++) {
			...
			headRow.append('<th title="' + wordday + '" class="ui-state-default ui-th-column ui-th-ltr DateHeader' + (weekday == 0 || weekday == 6 ? ' Weekend' : '') + '"><span>' + wordday + '</span></th>');
		}
